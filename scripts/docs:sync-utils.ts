...
async function syncModule(
  ...
  const write = (file: string, route: string, content: string) =>
    writeFileSync(
      join(moduleDir, file),
      banner(`${origin}${route}.md`) + transform(content, linkMap, alias),
    )
  ...
  slugs.forEach((s, i) => {
    const page = pages[i]
    if (!page) return
    write(`${s}.mdx`, `${routeBase}/${s}`, page)
    if (s === 'errors' || s === 'types') extras.push(s)
...
function writeSidebar(synced: Synced[]) {
  ...
  const content = `// AUTOGENERATED by \`pnpm docs:sync-utils\` — do not edit by hand.\nimport type { Sidebar } from 'vocs'\n\nexport const utilities = ${JSON.stringify(
    ...
    null,
    2,
  )} satisfies Sidebar[string][number]\n`
