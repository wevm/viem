...
import { AbiParameter, checksumAddress } from '../../src'
...
function generateValues(parameters: AbiParameter[]) {
  ...
  for (const parameter of parameters) {
    if (parameter.type.includes('[')) {
      ...
      const length = size ? parseInt(size) : Math.floor(Math.random() * 8)
      const nested = []
    ...
    if (parameter.type.startsWith('int') || parameter.type.startsWith('uint')) {
      ...
      const max =
        type === 'int'
          ? Math.random() > 0.5
            ? -(2n ** (BigInt(parseInt(size)) - 1n))
            : 2n ** (BigInt(parseInt(size)) - 1n) - 1n
          : 2n ** BigInt(parseInt(size)) - 1n
    ...
    if (parameter.type.startsWith('bytes')) {
      ...
      const value = bytesToHex(generateBytes(parseInt(size)))
      values.push(value)
      continue
    }
