...
test('moves USDC from an impersonated holder and stops impersonating', async () => {
  const before = {
    recipient: await balanceOf(recipient),
    whale: await balanceOf(whale),
  }
  const hash = await example()
  expect(await balanceOf(recipient)).toBe(before.recipient + 12_345_678n)
  expect(await balanceOf(whale)).toBe(before.whale - 12_345_678n)
