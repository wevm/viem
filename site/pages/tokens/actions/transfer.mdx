...
const { request, result } = await client.token.transfer.simulate({
  amount: { formatted: '1' },
  to: '0x742d35Cc6634C0532925a3b844Bc9e7595f0bEbb',
  token: 'usdc',
})

const receipt = await client.transaction.sendSync(request)
```
...
pass to [`sendCalls`](/docs/actions/wallet/sendCalls), [`multicall`](/docs/actions/public/multicall),
or any other Action that accepts a contract call.
...
The non-sync `transfer` action returns the transaction `Hex` hash instead.
