...
Viem provides type-safe, chain-aware Actions for interacting with tokens. Tokens are declared
on the [chain configuration](/docs/chains/introduction) (via the `tokens` property), and exposed
through a single `token` namespace on the Client.

Each Action selects its token either by `token` name (resolved to its token address and
`decimals` from the chain) or by an explicit token address, so you can call Actions like
...
on the active chain. See [Defining Tokens](/tokens/guides/defining-tokens) for
how to define reusable tokens and attach them to a chain. If the symbol is not
