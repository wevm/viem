...
The `viem/accounts`, `viem/actions`, `viem/ens`, `viem/nonce`, and `viem/siwe` subpath entrypoints were removed in favor of namespaces exported from the package root and `viem/utils`, and top-level types moved onto those namespaces (`Chain.Chain`, `Account.Account`). The flat `<Fn>Parameters`, `<Fn>ReturnType`, and `<Fn>ErrorType` type exports became `<namespace>.<fn>.Options`, `<namespace>.<fn>.ReturnType`, and `<namespace>.<fn>.ErrorType`. See [Entrypoints & Exports](#entrypoints--exports).
...
Flat utility functions moved onto Ox-backed primitive namespaces (`Hex`, `Bytes`, `Value`, `AbiParameters`, `Signature`, ...) exported from `viem/utils` (not the package root), recovery and verification helpers became synchronous (no `await`), and `abitype` types are imported from `abitype` directly instead of being re-exported. See [Utilities](#utilities).
...
```diff
+ import { AbiError, CcipRead } from 'viem/utils'
+ import { Actions } from 'viem'
...
```diff
+ import { AbiConstructor, AbiError, AbiEvent, AbiFunction, AbiItem } from 'viem/utils'
...
```diff
+ import { Blobs, type TxEnvelopeEip4844 } from 'viem/utils'
...
+ import { BlockOverrides, Filter, StateOverrides } from 'viem/utils'
...
```diff
+ import { PersonalMessage, TxEnvelope, TypedData } from 'viem/utils'
...
```diff
+ import { Hex, PersonalMessage, TypedData } from 'viem/utils'
...
```diff
+ import { AccessList, Authorization } from 'viem/utils'
...
```diff
+ import { Authorization, Bytes } from 'viem/utils'
