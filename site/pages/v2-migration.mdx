...
CCIP Read is disabled by default and requires an explicit client policy. A tunnel with a trusted HTTPS batch gateway is recommended.

Resolution now occurs inside contract calls, not only ENS lookups. See [ENS & CCIP Read](#ens--ccip-read).

```ts
import { Client, http } from 'viem'
import { CcipRead } from 'viem/utils' // [!code ++]

const client = Client.create({
  ccipRead: CcipRead.tunnel({ // [!code ++]
    batchGateways: ['https://ccip-v3.ens.xyz'], // [!code ++]
  }), // [!code ++]
  transport: http(),
})
```
...
CCIP Read is disabled by default and requires an explicit client policy. It now resolves inside calls for any contract.

A tunnel with a trusted HTTPS batch gateway is recommended. Direct requests provide defense in depth only.

Server applications should use a trusted proxy or gateway allowlist. Portable fetch implementations cannot pin DNS results to connections.
...
Test actions moved from flat exports into root test actions and grouped client decorator namespaces.
...
await Actions.test.block.mine(client, { blocks: 1 }) // [!code ++]
await Actions.test.address.setBalance(client, { address, value }) // [!code ++]
...
await Actions.test.block.mine(client, { blocks: 1, mode: 'hardhat' }) // [!code ++]
```
...
Actions.erc7821.ExecuteUnsupportedError // [!code ++]
Actions.erc7821.FunctionSelectorNotRecognizedError // [!code ++]
...
import { SoladySmartAccount } from 'viem/account-abstraction' // [!code ++]
...
const manager = NonceManager.from({ source: NonceManager.jsonRpc() }) // [!code ++]
```
...
import { Client, PaymasterClient, http } from 'viem/account-abstraction' // [!code ++]
import { RpcSchema } from 'viem/utils' // [!code ++]
...
const bundler = Client.create({ // [!code ++]
  schema: RpcSchema.from<{ // [!code ++]
...
import { Credential } from 'viem/account-abstraction' // [!code ++]
...
import type { SmartAccount } from 'viem/account-abstraction' // [!code ++]
