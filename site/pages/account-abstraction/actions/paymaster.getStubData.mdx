...
const paymasterArgs = await paymasterClient.paymaster.getStubData({
...
Paymaster-related User Operation properties, plus optional `isFinal` and `sponsor` metadata. EntryPoint 0.6 returns `paymasterAndData`; EntryPoint 0.7, 0.8, and 0.9 return split Paymaster fields.
