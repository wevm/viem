...
const { request } = await l1Client.opStack.buildInitiateWithdrawal({
  account,
  to: account, // [!code focus]
  value: Value.fromEther('1'), // [!code focus]
})

const hash = await l2Client.opStack.initiateWithdrawal({
  request, // [!code focus]
})
// @log: 0xf8e81c7d...
```
...
const { game, withdrawal } = await l1Client.opStack.waitToProve({
  receipt, // [!code focus]
  targetChain: optimism, // [!code focus]
})

const proof = await l2Client.opStack.buildProveWithdrawal({
  game, // [!code focus]
  withdrawal, // [!code focus]
})

await l1Client.opStack.proveWithdrawal({
  ...proof,
  targetChain: optimism,
})

await l1Client.opStack.waitToFinalize({
  targetChain: optimism,
  withdrawalHash: withdrawal.withdrawalHash,
})

const finalizeHash = await l1Client.opStack.finalizeWithdrawal({
  targetChain: optimism,
  withdrawal,
})
// @log: 0x9b47d3fa...
