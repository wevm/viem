...
Create one with [`NonceManager.from`](#noncemanagerfrom), backed by a source that seeds nonces (such as [`NonceManager.jsonRpc`](#noncemanagerjsonrpc), which reads the pending nonce via `eth_getTransactionCount`). A ready-made instance backed by the JSON-RPC source is exported as `NonceManager.nonceManager`.
...
const account = Account.fromPrivateKey('0x...', {
  nonceManager: NonceManager.nonceManager, // [!code focus]
})
...
// ---cut---
const nonceManager = NonceManager.from({
  source: NonceManager.jsonRpc(), // [!code focus]
})
```
...
Creates a JSON-RPC source for a nonce manager that reads the pending nonce via `eth_getTransactionCount`.
...
A source that seeds nonces from the pending transaction count of the address. Writes are no-ops (the chain itself tracks used nonces).
