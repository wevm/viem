...
[`Engine.install`](/docs/engine/install) resolves an Engine and installs its slots atomically. The
`viem/wasm` and `viem/node` entrypoints prepare and install their complete provider Engines with no
arguments. The root `viem` entrypoint accepts custom or previously prepared slots.
