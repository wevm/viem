...
Definitions are also minimal: only `id` is required, and `rpcUrls` and `blockExplorers` hold a single flattened entry instead of `default`-keyed maps, with `http` and `ws` accepting one URL or a list.
...
The chain shape slimmed down. `name`, `nativeCurrency`, and `rpcUrls` are optional (only `id` is required), and a chain without `rpcUrls` requires an explicit URL on the Transport.

`rpcUrls` and `blockExplorers` are single flattened entries instead of `default`-keyed maps. `rpcUrls.http` (and the renamed `ws`) accept one URL or a list; keyed alternates (e.g. a `blockscout` explorer entry) have no equivalent, so keep the primary entry.
