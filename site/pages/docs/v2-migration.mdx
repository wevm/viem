...
Block watchers (`watchBlocks`, `watchBlockNumber`) and the block filter producer moved with the
...
The message and typed-data signing actions stay flat on `Actions`, as do the onchain verification
actions. [`verifySiweMessage`](/docs/actions/public/verifySiweMessage) moved out of `viem/siwe` to
join them.
