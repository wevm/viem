...
Treat Accounts, Chains, Transports, and errors as version-specific. Share version-neutral
boundaries such as RPC URLs, EIP-1193 providers, serialized values, or plain data.

### Adapt Clients Between Versions

Use `Client.toV2` to create a v2-compatible base Client from a v3 Client. Extend it with v2's
native action decorators:
...
Use `Client.fromV2` in the other direction, then extend the result with v3 decorators:
...
The adapters reuse request connectivity and preserve JSON-RPC Accounts and `ccipRead: false`. They
do not translate local Accounts, custom extensions, subscriptions, Chain codecs or formatters,
custom CCIP Read handlers, or error identities. Pass a native target-version Chain or Account when
...
Contract instances now bind one client through `Contract.from`. Event reads were renamed, while function arguments remain the first positional parameter.
...
Low-level request composition was internalized. Compose requests at the Client or Transport boundary. Action dispatch resolves through [`Actions.getAction`](/docs/clients/override), which consults actions attached to the Client via `.extend()`, so `getAction`-style overrides carry over.
