...
connectivity while an application runs Viem v2 and v3 together. Each adapter returns a base Client
that can be extended with the target version's native action decorators.
...
The adapters preserve request connectivity, JSON-RPC Accounts, and `ccipRead: false`.
`Client.fromV2` preserves the source chain ID, while `Client.toV2` creates a chainless Client by
default.

Local Accounts are not carried automatically. Custom extensions, subscriptions, Chain codecs and
formatters, custom CCIP Read handlers, and error identities are not translated across the version
boundary. Pass a native target-version Chain or Account when that behavior is required.

## Recipes

These recipes assume Viem v3 is installed as `viem` and Viem v2 is aliased as `viem-v2`.
...
| Error | Description |
| --- | --- |
| `Account.InvalidAddressError` | The `account` override was an address string that failed validation. |
...
`Client.toV2` does not throw while adapting a Client. Requests through the returned Client surface
the source v3 Client's errors without translating their identities.
