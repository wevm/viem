...
- **Type:** `Omit<Actions.contract.watchEvent.Options, 'abi' | 'address' | 'eventName'>`
- **Optional**

Options include `args`, `batch`, `fromBlock`, `poll`, `pollingInterval`, and `strict`. Logs are batched by default, and `strict` defaults to `false`.

When `poll` is omitted, `fromBlock` selects polling. Without `fromBlock`, the method subscribes when the [Transport](/docs/transports) supports subscriptions and polls otherwise.
