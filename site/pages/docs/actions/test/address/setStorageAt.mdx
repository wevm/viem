...
Writes a 32-byte value to an account's storage slot.
...
## `Actions.address.setStorageAt`

Writes to a slot of an account's storage.

### Usage

Call [`Actions.address.setStorageAt`](#actionsaddresssetstorageat) directly by passing the Client as the first argument.
...
- **Type:** `Actions.address.setStorageAt.Options`
