...
const result = await client.txpool.getStatus() // [!code focus]
...
## `Actions.txpool.getStatus`

Returns the number of transactions currently pending for inclusion in the next blocks, as well as the ones being scheduled for future execution.

### Usage

Call [`Actions.txpool.getStatus`](#actionstxpoolgetstatus) directly by passing the Client as the first argument.
