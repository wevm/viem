...
### Call Without Sending a Transaction
...
Call [`Actions.call`](#actionscall) directly by passing the Client as the first argument.
...
The Account object or address to use as the call's `msg.sender`. This option takes precedence over [`from`](#optionsfrom).
...
The factory that deploys the contract before the call. Pass this option with [`factoryData`](#optionsfactorydata). It cannot be combined with [`code`](#optionscode).
...
The calldata that deploys the contract through [`factory`](#optionsfactory). Pass both options together.
