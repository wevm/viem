...
The `pending` block tag is supported, but discovery and balance measurement use
separate requests. Pending state can change between them, so the results are not pinned
to one pending snapshot.
...
Two kinds of balance are not traced:

- ERC1155. Its `balanceOf` takes a token id, so those balances cannot be read by the same
  probe.
- Tokens whose balance changes without emitting a `Transfer` that the account
  participates in (some rebasing and non-standard tokens).
