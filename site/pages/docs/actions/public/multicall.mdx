...
Viem caches the selected mode for each Client. Set [`mode`](#optionsmode) to choose a specific mode.
...
Trace the account's native and token balance changes across the batch with [`traceAssetChanges`](#optionstraceassetchanges):
...
Call [`Actions.multicall`](#actionsmulticall) directly by passing the Client as the first argument.
...
Account attached to the calls (`msg.sender` on the `eth_simulateV1` path). Required for [`traceAssetChanges`](#optionstraceassetchanges).
...
#### options.traceAssetChanges

- **Type:** `boolean`

Whether to trace native/ERC20/ERC721 balance changes of [`account`](#optionsaccount) across the batch. Requires `account`; forces the `eth_simulateV1` mode.
...
Per-call `results` (in call order): `{ data, gasUsed, logs, result, status: 'success' } | { data, gasUsed, logs, error, status: 'failure' }`. With [`allowFailure: false`](#optionsallowfailure), `results` holds bare decoded values instead.
...
`assetChanges` is populated when [`traceAssetChanges`](#optionstraceassetchanges) is set.
