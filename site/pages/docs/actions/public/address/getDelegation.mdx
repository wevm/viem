...
const delegation = await client.address.getDelegation({ // [!code focus]
...
## `Actions.address.getDelegation`

Returns the address that an account has delegated to via [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702), or `undefined` if the account is not delegated.

### Usage

Call [`Actions.address.getDelegation`](#actionsaddressgetdelegation) directly by passing the Client as the first argument.
...
- **Type:** `Actions.address.getDelegation.Options`
...
// ---cut---
const delegation = await Actions.address.getDelegation(client, { // [!code focus]
  address: '0xA0Cf798816D4b9b9866b5330EEa46a18382f251e', // [!code focus]
}) // [!code focus]
```
