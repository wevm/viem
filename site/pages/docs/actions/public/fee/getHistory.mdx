...
const feeHistory = await client.fee.getHistory({ // [!code focus]
...
## `Actions.fee.getHistory`

Returns a collection of historical gas information.

### Usage

Call [`Actions.fee.getHistory`](#actionsfeegethistory) directly by passing the Client as the first argument.
...
Highest number block of the requested range. Mutually exclusive with [`blockTag`](#optionsblocktag).
...
Highest number block of the requested range. Mutually exclusive with [`blockNumber`](#optionsblocknumber).
