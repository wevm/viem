...
## `Actions.fee.estimateFeesPerGas`

Returns an estimate for the fees per gas (in wei) for a transaction to be likely included in the next block.

### Usage

Call [`Actions.fee.estimateFeesPerGas`](#actionsfeeestimatefeespergas) directly by passing the Client as the first argument.
...
- **Type:** `Actions.fee.estimateFeesPerGas.Options`
...
// ---cut---
const fees = await Actions.fee.estimateFeesPerGas(client, { // [!code focus]
