...
Unlike [write](/docs/actions/wallet/contract/write), this validates the call against current state (surfacing return data and revert reasons) via `eth_call`, so it does not require gas or change chain state.
