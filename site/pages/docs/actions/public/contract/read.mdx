...
Call [`Actions.contract.read`](#actionscontractread) directly by passing the Client as the first argument.
...
Reads the contract at the block with the given hash ([EIP-1898](https://eips.ethereum.org/EIPS/eip-1898)). Pair with [`requireCanonical`](#optionsrequirecanonical) to reject blocks outside the canonical chain.
...
The factory address for a deployless read. Pass [`factoryData`](#optionsfactorydata) and the counterfactual contract `address`.
...
Whether the block given via [`blockHash`](#optionsblockhash) must be in the canonical chain ([EIP-1898](https://eips.ethereum.org/EIPS/eip-1898)).
...
The [state overrides](/docs/actions/public/call#optionsstateoverride) to apply for the read.
