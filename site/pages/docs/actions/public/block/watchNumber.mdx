...
## `Actions.block.watchNumber`

Watches incoming block numbers, returning a watcher handle.

### Usage

Call [`Actions.block.watchNumber`](#actionsblockwatchnumber) directly by passing the Client as the first argument.
...
- **Type:** `Actions.block.watchNumber.Options`
...
// ---cut---
const watch = Actions.block.watchNumber(client, { // [!code focus]
  emitMissed: true, // [!code focus]
}) // [!code focus]
watch.onBlockNumber((blockNumber) => console.log(blockNumber))
```
