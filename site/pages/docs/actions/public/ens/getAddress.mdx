...
## `Actions.ens.getAddress`

Gets the address for an ENS name (via the ENS Universal Resolver's `resolveWithGateways`).

### Usage

Call [`Actions.ens.getAddress`](#actionsensgetaddress) directly by passing the Client as the first argument.
...
An explicit [`coinType`](#optionscointype) uses the `bytes` resolver profile, which returns an address without checksum casing.

### Errors

| Error | Description |
| --- | --- |
| `ContractError.ContractFunctionExecutionError` | The Universal Resolver call failed (with [`strict`](#optionsstrict); resolver errors otherwise map to `null`). |
