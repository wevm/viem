...
const changes = await client.filter.getChanges({ filter }) // [!code focus]
// @log: [
...
## `Actions.filter.getChanges`

Returns the changes for a filter since it was created or last polled.

### Usage

Call [`Actions.filter.getChanges`](#actionsfiltergetchanges) directly by passing the Client as the first argument.
...
- **Type:** `Actions.filter.getChanges.Options`
...
const changes = await Actions.filter.getChanges(client, { filter }) // [!code focus]
```
