...
const logs = await client.filter.getLogs({ filter }) // [!code focus]
// @log: [
// @log:   {
...
## `Actions.filter.getLogs`

Returns the logs matching an event filter.

### Usage

Call [`Actions.filter.getLogs`](#actionsfiltergetlogs) directly by passing the Client as the first argument.
...
- **Type:** `Actions.filter.getLogs.Options`
...
const logs = await Actions.filter.getLogs(client, { filter }) // [!code focus]
```
