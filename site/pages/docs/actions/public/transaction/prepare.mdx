...
| Error | Description |
| --- | --- |
| `Errors.fee.Eip1559FeesNotSupportedError` | `maxFeePerGas`/`maxPriorityFeePerGas` were supplied for a `legacy`/`eip2930` transaction. |
| `Errors.transaction.MaxFeePerGasTooLowError` | The supplied `maxFeePerGas` is lower than the estimated `maxPriorityFeePerGas`. |
| `RpcError` | The node rejected one of the underlying RPC requests. |
