...
## `Actions.transaction.prepare`

Prepares a transaction request for signing, by populating the fields required to be signed over (for example, `nonce`, `chainId`, `type`, fees, `gas`).

### Usage

Call [`Actions.transaction.prepare`](#actionstransactionprepare) directly by passing the Client as the first argument.
...
The blobs to attach to the transaction (EIP-4844). When supplied together with [`kzg`](#optionskzg), `blobVersionedHashes` (and optionally `sidecars`) are derived.
...
The KZG context, used to derive blob fields (`blobVersionedHashes`, `sidecars`) from [`blobs`](#optionsblobs).
...
The nonce to use for the transaction. When provided, it takes precedence over the [`nonceManager`](#optionsnoncemanager).
...
A nonce manager used to derive the transaction nonce when [`nonce`](#optionsnonce) is not provided.
