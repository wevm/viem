...
- **Type:** `Actions.transaction.prepare.Options`
...
// ---cut---
const { request } = await Actions.transaction.prepare(client, { // [!code focus]
...
The Account or address that sends the transaction (`from`). Defaults to the Client's [`account`](/docs/clients/create#account).
