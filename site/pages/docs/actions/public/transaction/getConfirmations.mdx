...
const confirmations = await client.transaction.getConfirmations({ // [!code focus]
...
## `Actions.transaction.getConfirmations`

Returns the number of blocks passed (confirmations) since the transaction was processed on a block.

### Usage

Call [`Actions.transaction.getConfirmations`](#actionstransactiongetconfirmations) directly by passing the Client as the first argument.
...
#### options.transactionReceipt

- **Type:** `TransactionReceipt`

Compute confirmations from an already-fetched [transaction receipt](/docs/actions/public/transaction/getReceipt), saving an extra `eth_getTransactionByHash` request. Mutually exclusive with [`hash`](#optionshash).
