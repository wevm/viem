...
Whether to [prepare](/docs/actions/public/transaction/prepare) the transaction request before signing (populating `nonce`, `chainId`, fees, `gas`, and `type`). When `false`, the request is signed as-is.
