...
## `Actions.event.createFilter`

Creates a filter to listen for new event logs.

### Usage

Call [`Actions.event.createFilter`](#actionseventcreatefilter) directly by passing the Client as the first argument.
...
- **Type:** `Actions.event.createFilter.Options`
