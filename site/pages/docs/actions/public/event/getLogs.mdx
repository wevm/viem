...
## `Actions.event.getLogs`

Returns a list of event logs matching the provided parameters.

### Usage

Call [`Actions.event.getLogs`](#actionseventgetlogs) directly by passing the Client as the first argument.
...
- **Type:** `Actions.event.getLogs.Options`
...
const client = Client.create({ chain: mainnet, transport: http() })
// ---cut---
const logs = await Actions.event.getLogs(client, { // [!code focus]
  address: '0xfba3912ca04dd458c843e2ee08967fc04f3579c2', // [!code focus]
}) // [!code focus]
```
