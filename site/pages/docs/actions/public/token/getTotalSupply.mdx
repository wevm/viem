...
const supply = await client.token.getTotalSupply({ // [!code focus]
...
## `Actions.token.getTotalSupply`

Gets the total supply of an ERC-20 token, returned in base units and human-readable form.

### Usage

Call [`Actions.token.getTotalSupply`](#actionstokengettotalsupply) directly by passing the Client as the first argument.
...
- **Type:** `Actions.token.getTotalSupply.Options`
...
// ---cut---
const supply = await Actions.token.getTotalSupply(client, { // [!code focus]
  token: '0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', // [!code focus]
