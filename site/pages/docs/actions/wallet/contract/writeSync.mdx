...
## `Actions.contract.writeSync`

Executes a write (`nonpayable`/`payable`) function on a contract synchronously, and returns the transaction receipt.

### Usage

Call [`Actions.contract.writeSync`](#actionscontractwritesync) directly by passing the Client as the first argument.
...
- **Type:** `Actions.contract.writeSync.Options`
