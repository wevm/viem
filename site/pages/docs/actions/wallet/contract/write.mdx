...
Calls a write function and broadcasts the resulting transaction.
...
## `Actions.contract.write`

Executes a write (`nonpayable`/`payable`) function on a contract, and returns the transaction hash.

### Usage

Call [`Actions.contract.write`](#actionscontractwrite) directly by passing the Client as the first argument.
...
before broadcasting.
Pass the returned `request` to [`Actions.contract.write`](#actionscontractwrite).
...
- **Type:** `Actions.contract.write.Options`
