...
const success = await client.wallet.watchAsset({ // [!code focus]
...
## `Actions.wallet.watchAsset`

Adds an EVM token to the wallet's watchlist so its balance is tracked.

### Usage

Call [`Actions.wallet.watchAsset`](#actionswalletwatchasset) directly by passing the Client as the first argument.
...
- **Type:** `Actions.wallet.watchAsset.Options`
