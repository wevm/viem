...
Call [`Actions.transaction.send`](#actionstransactionsend) directly by passing the Client as the first argument.
...
The blobs to attach to the transaction (EIP-4844). Use with [`kzg`](#optionskzg) to derive the blob fields.
...
The chain the transaction targets. Pass `null` to skip the current-chain assertion (see [`assertChainId`](#optionsassertchainid)).
...
The KZG context used to derive EIP-4844 blob fields from [`blobs`](#optionsblobs).
