...
This example approves a spender to transfer an amount of an ERC-20 token on behalf of the caller, and returns the transaction hash.
...
Call `Actions.token.approve` directly by passing the Client as the first argument.
...
:::tip
Use [`Actions.token.approveSync`](/docs/actions/wallet/token/approveSync) to wait for the transaction receipt and decoded `Approval` event in a single call.
:::
