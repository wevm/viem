...
- **Paymasters:** Allow third parties to pay for gas fees, or pay for gas via ERC20 tokens.
...
You can also leverage the `userOperation.sponsorUserOperation` property on the Bundler Client to hoist Paymaster capabilities such as User Operation gas fee sponsorship.

In the example below, we are utilizing [Pimlico's Paymaster `pm_sponsorUserOperation` API](https://docs.pimlico.io/infra/paymaster) to sponsor & cover the gas fees that the User Operation consumes.
...
const paymasterClient = createClient({ // [!code ++]
...
const bundlerClient = createBundlerClient({
  ...
  transport: http('https://public.stackup.sh/api/v1/node/ethereum-mainnet'),
  userOperation: { // [!code ++]
    async sponsorUserOperation({ account, userOperation }) { // [!code ++]
      const result = await paymasterClient.request({ // [!code ++]
        method: 'pm_sponsorUserOperation', // [!code ++]
        params: [ // [!code ++]
          formatUserOperationRequest(userOperation), // [!code ++]
          account?.entryPoint.address, // [!code ++]
        ], // [!code ++]
      }) // [!code ++]
      return formatUserOperation(result) // [!code ++]
    }, // [!code ++]
  }, // [!code ++]
})
