...
Quote token updates are a two-step flow: this action stages the next quote token, then [`token.updateQuoteToken`](/tempo/actions/token.updateQuoteToken) completes the update.
