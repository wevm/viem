...
includes `stage` and `state` values that can be passed back through `resume`.
...
and must match the owner address.
...
block or the first attempted stack deployment block to avoid an unbounded history query.
...
values are not accepted because each stage is a separate transaction. Use the primitive actions when
