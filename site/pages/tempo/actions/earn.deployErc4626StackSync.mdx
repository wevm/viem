...
Canonical Tempo factory addresses are available from the chain's `contracts` metadata. Pass the
pair explicitly so the deployment release remains visible at the call site.
...
Explicit engine and Earn factory addresses from one release. Canonical Tempo addresses are
available from `client.chain.contracts`.
