...
Indexed argument values to filter logs by. Derive a role hash from a named role with `TokenRole.serialize` from `ox/tempo/TokenRole`, for example `TokenRole.serialize('pause')`.
