...
The action reads the input token from the pool and batches its approval with the swap. `propAmm.swap.call` defines only the raw swap call for manual composition and requires explicit `baseToQuote`, `customerId`, `deadline`, `recipient`, and `oraclePriceToleranceBps` values.
...
- **Type:** `Hex`
- **Default:** The sender address left-padded to 32 bytes
