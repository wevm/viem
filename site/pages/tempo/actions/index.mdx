...
Zone RPC helpers are available on a derived zone client created with
[`getZoneClient`](/tempo/client) via `zoneClient.zone.*`.

They are also exported as free functions via `Actions.zone.*` and `zoneActions.*`.

The derived zone client also exposes Tempo token helpers on `zoneClient.token.*`.

| Action | Description |
|--------|-------------|
| **Zone Actions** | |
| [`zone.getAuthorizationTokenInfo`](/tempo/actions/zone.getAuthorizationTokenInfo) | Returns the authenticated account address and authorization token expiry |
| [`zone.getZoneInfo`](/tempo/actions/zone.getZoneInfo) | Returns the current zone metadata |
| [`zone.getDepositStatus`](/tempo/actions/zone.getDepositStatus) | Returns deposit processing status for a given Tempo block number |
| **Access Key Actions** | |
