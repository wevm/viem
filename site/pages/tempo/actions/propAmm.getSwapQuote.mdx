...
const [amountOut, oraclePrice, oracleUpdatedAt, creditAfter] = await client.propAmm.getSwapQuote({
  amountIn: 1_000_000n,
  baseToQuote: true,
...
// @log: [amountOut, oraclePrice, oracleUpdatedAt, creditAfter]
```
...
For exact input, `amount` is the quoted output. For exact output, it is the required input. The other values bind the oracle observation and report the route's remaining rounding credit.
...
Address that will submit the swap and supply the input token.
