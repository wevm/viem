...
Role values are bytes32 role hashes. Compare them against a named role (`defaultAdmin`, `pause`, `unpause`, `issuer`, `burnBlocked`) with `TokenRole.serialize` from `ox/tempo/TokenRole`.
...
Indexed argument values to filter logs by. Derive a role hash from a named role with `TokenRole.serialize` from `ox/tempo/TokenRole`, for example `TokenRole.serialize('issuer')`.
