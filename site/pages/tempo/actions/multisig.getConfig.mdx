...
Configure [`experimental_multisig`](/tempo/utilities/Multisig#client-setup) on the client so the
request reaches a multisig coordinator.
...
An address-only account contains no local owner keys. Pass the owner that signs the approval through
`account`.
