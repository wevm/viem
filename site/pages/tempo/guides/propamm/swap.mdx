...
[`swapSync`](/tempo/actions/propAmm.swapSync). The action approves the pool and swaps in one Tempo
...
[`swapSync`](/tempo/actions/propAmm.swapSync) with `amountOut` and `maxAmountIn`. The action approves
