...
2. Prepare the transaction request, passing the multisig `account`.
3. Each owner signs the prepared request to produce an **owner approval** signature.
4. Broadcast synchronously with the multisig `account` and collected `signatures`.

The first transaction from a multisig account **auto-bootstraps** (registers) it onchain,
...
// 4. Broadcast with the multisig account and collected signatures. // [!code focus]
