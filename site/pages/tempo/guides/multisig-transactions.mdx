...
// 1. Define the multisig configuration (2-of-2) and derive the account.
...
// 2. Prepare the request, passing the multisig config.
...
// 3. Each owner approves the prepared request. Spread `...request` first so the
//    explicit `account` wins (the request carries no signing account).
...
// 4. Broadcast with the multisig account and the collected signatures.
...
:::tip
The first transaction from a multisig account automatically bootstraps (registers) it
on-chain – no explicit `init` flag is required. Subsequent transactions are sent the same way.
:::
...
// Only 2 of the 3 owners need to approve to meet the threshold.
const signature_1 = await client.signTransaction({ ...request, account: owner_1 }) // [!code focus]
