...
Preflight checks require the binding signer before deployment starts. The owner must have enough of
the selected fee token to submit the binding transaction.
...
try {
  ...
  if (error instanceof Actions.earn.DeployErc4626StackError) {
    console.log(error.stage, error.state)
...
Each primitive exposes a `call` helper for `sendCalls`, simulation, and gas estimation. Treat
multi-call deployment as experimental. Tempo limits contract creation per transaction, and the
combined gas profile must be measured against the target network before batching.
...
Viem generates its Earn ABIs from the pinned `tempoxyz/earn` source revision. Factory deployment
bytecode is not exposed as a public action: deploying and reviewing factory releases remains an Earn
release process.
