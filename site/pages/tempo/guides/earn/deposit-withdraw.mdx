...
builder takes the asset token contract address and returns both the token approval and vault action.
Simulate the complete batch with
[`block.simulate`](/docs/actions/public/block/simulate), since `earn.deposit.simulate` assumes the
...
const [simulation] = await client.block.simulate({
  blocks: [{ calls }],
  validation: true,
})

if (simulation.calls.some((result) => result.status === 'failure'))
  throw new Error('Earn batch simulation failed.')

const receipt = await client.transaction.sendSync({ calls })
