...
Use [`earn.depositSync`](/tempo/actions/earn.deposit) with a current Earn share quote and a slippage
...
Set `recipient` when the account supplying assets should mint Earn shares to another address. This
...
Use [`earn.getPosition`](/tempo/actions/earn.getPosition) to read the account's current Earn shares
...
Earn shares are non-rebasing. Their count can stay fixed while their redeemable asset value changes.
...
### Redeem Directly to Venue Shares

Use [`earn.redeemSharesSync`](/tempo/actions/earn.redeemShares) to leave an Earn vault without
unwinding the underlying venue position. The vault must report `capabilities.inKindRedeem: true`.
This is useful when moving between Earn vaults backed by the same ERC-4626 venue: redeem venue
shares from the old vault, then pass them to `earn.depositSharesSync` on the replacement vault.
...
// @log: { venueShareAmount: 99_000_000n, venueShareToken: '0x...', ... }
```

The transaction burns the caller's Earn shares and sends the corresponding venue shares to
`recipient`. It does not automatically enter another Earn vault. Integrations can compose the
returned venue shares with a separate deposit after the first transaction confirms.
