...
A protected Earn vault attaches an exit-safe TIP-403 policy to its Earn share token. The policy
allows every sender, while one whitelist controls who can receive transfers and newly minted Earn
shares.
...
The always-allow sender rule preserves the Earn share side of an exit. Keep the `EarnVault` and
every other required protocol recipient eligible so an existing holder can transfer shares through
the vault and redeem.
...
These recipes assume you have [set up a Tempo client](/tempo) whose account can change the Earn
...
policy. Include every address that must receive Earn shares during normal operation.
...
const earnFees = '0x0000000000000000000000000000000000000003'
const earnRouter = '0x0000000000000000000000000000000000000004'
const portal = '0x0000000000000000000000000000000000000005'
const messenger = '0x0000000000000000000000000000000000000006'
const outbox = '0x0000000000000000000000000000000000000007'
const feeRecipient = '0x0000000000000000000000000000000000000008'
...
const { shareToken } = await client.earn.getVault({ vault: earnVault })
...
Required members include the `EarnVault`, its `EarnFees` contract, the selected Earn router, Zone
Portal, messenger, outbox, and every active fee recipient. Add any deployment-specific receiver
that may receive Earn shares. Store all four returned policy IDs with the deployment record.
...
`recipient` receives the Earn shares.
...
Only the Earn share recipient must pass this eligibility policy. Asset-token policies, receive
...
Pass `allowed: false` to revoke admission. Do not remove the `EarnVault`, `EarnFees` contract,
selected Earn router, Portal, messenger, outbox, active fee recipient, or a receiver with an open
asynchronous claim. Revalidate all required protocol members after every change.

### Exit After Eligibility Removal

A removed holder cannot receive more Earn shares, but the exit-safe sender policy still lets it read
...
This preserves the Earn share policy side of the exit. The vault must still support the requested
...
Maintain a deployment-specific list of every protocol address that receives Earn shares. A generic
