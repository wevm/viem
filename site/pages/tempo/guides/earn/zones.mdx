...
The router uses the vault's `depositSwapOverride` for `assetToken` when configured, otherwise it
uses Tempo's canonical stablecoin DEX. Omitting `vaultAssetAmountMin` accepts any non-reverting swap
output, so set a deliberate bound.
...
Set `assetToken` to the output token's contract address to swap the redeemed vault asset before
returning it to the Zone. For an alternate output token, `assetAmountMin` bounds the final post-swap
amount.
...
The router uses the vault's `redeemSwapOverride` for `assetToken` when configured, otherwise it uses
Tempo's canonical stablecoin DEX. A live vault quote cannot quote the swap output, so provide
`assetAmountMin` or an alternate-asset quote with `slippageBps`.
...
Earn shares, approve them to the Zone Portal, and deposit them to an encrypted recipient in one
atomic transaction.
...
const earnShareAmount = await parentClient.contract.read({
  abi: Abis.earnVault,
  address: vaultAddress,
  args: [venueShareAmount],
  functionName: 'convertEngineSharesToEarnShares',
})
const encrypted = await Actions.zone.encryptedDeposit.prepare(parentClient, {
...
const calls = [
  ...
  ...Actions.zone.encryptedDeposit.calls(encrypted),
]

const receipt = await parentClient.transaction.sendSync({ calls })
// @log: { transactionHash: '0x1234...abcd', ... }
```
