...
Pass the Zone asset's contract address as `assetToken` when it differs from the vault asset. The
gateway swaps it before the deposit. Set `vaultAssetAmountMin` to bound the vault assets produced by
that swap independently from the vault share bound.
