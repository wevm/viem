...
The vault transaction and bounceback address are public. The recipient of the returned Zone deposit is
...
but the action amount, timing, gateway, and bounceback address remain public at the privacy boundary.
...
The vault action, amounts, gateway, and bounceback address are visible on the parent chain. The Zone
recipient and balances returned to the Zone remain private.

### Use a Safe Bounceback Recipient

`bouncebackRecipient` receives funds on the public Tempo chain if the encrypted return deposit
