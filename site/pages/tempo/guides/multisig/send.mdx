...
- **[Local signing](#local-signing):** Use [`transaction.sendSync`](/docs/actions/wallet/transaction/sendSync) when the
...
`transaction.sendSync`. The multisig account signs a complete quorum locally.
...
Normal `transaction.send` also works when a prepared request already contains enough valid
...
const pending = await client.transaction.sendSync({
  account: multisig,
  calls: [{ data: '0xdeadbeef', to: '0xcafebabecafebabecafebabecafebabecafebabe' }],
  owner: owner_1,
})
...
const pending2 = await client.transaction.sendSync({
  account: multisig,
...
[`transaction.send`](/docs/actions/wallet/transaction/send) always returns the operation hash.
[`transaction.sendSync`](/docs/actions/wallet/transaction/sendSync) returns a pending receipt below
...
While quorum is pending, `transaction.get` returns a synthetic transaction. Its `hash` is the
...
const { request } = await CoreActions.transaction.prepare(client, {
  account: multisig,
  calls: [{ data: '0xdeadbeef', to: '0xcafebabecafebabecafebabecafebabecafebabe' }],
})
...
const signatures = await Promise.all(
  [owner_1, owner_2].map((account) =>
    CoreActions.transaction.sign(client, { ...request, account })
  )
)
