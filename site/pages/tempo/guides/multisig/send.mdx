...
[`Storage.memory`](/tempo/utilities/Storage.memory) store is process-local and is not suitable for
...
The first owner passes the transaction fields and multisig account. Each later owner passes only
...
quorum and the submitted receipt when an approval reaches quorum.
