...
[`transaction.sendSync`](/docs/actions/wallet/transaction/sendSync). The account creates the complete
...
[`transaction.sendSync`](/docs/actions/wallet/transaction/sendSync). The first owner passes the
...
const receipt = await CoreActions.transaction.sendSync(client, { ...request, signatures })
```
