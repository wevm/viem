...
you call `transaction.sign`. Address-only owners support distributed signing, where each owner
...
The account's `transaction.sign` method combines retained local owner approvals with any approvals
