...
Creates a scope builder for an arbitrary target. Use `target.any()` to allow any selector on the target, or `target.selector(selector, options)` to allow a specific selector.
...
Selector map used by [`Scopes.contract`](#scopescontract). Generated selector maps are exported from [`Selectors`](/tempo/utilities/Selectors).
