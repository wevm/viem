...
`withRelay` forwards every `eth_fillTransaction` request to the relay and preserves the request's
...
The relay must handle the `multisig_*` methods and proxy ordinary transaction and receipt lookups
to its upstream RPC. Viem sends `eth_getTransactionByHash` and `eth_getTransactionReceipt` through
the relay because a pending operation hash exists only in the relay's store.
...
const handler = Handler.relay({ // [!code hl]
  feePayer: { // [!code hl]
    account: Account.fromPrivateKey('0x...'), // [!code hl]
  }, // [!code hl]
}) // [!code hl]
...
```ts
type ReturnType = Transport<'relay', { multisig: true }>
```
...
The relay transport to use for fee sponsorship and multisig coordination.
