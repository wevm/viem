...
wrapped transport when the account reaches quorum. All unrelated RPC requests pass through unchanged.
...
Storage shared by every coordinator for the same multisig accounts. Production stores should
implement atomic `compareAndSet` to prevent concurrent approvals from overwriting each other.
