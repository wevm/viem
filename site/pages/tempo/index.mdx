...
Chains with configured zones expose `getZoneClient`, which returns a wallet-capable
zone client. Public and wallet actions remain available directly on the derived
client, Tempo token helpers are available on `zoneClient.token.*`, and zone RPC
helpers are available on `zoneClient.zone.*`.
...
const zoneBalance = await zoneClient.token.getBalance({
  token: '0x20c0000000000000000000000000000000000000',
})

console.log('Zone ID:', zoneInfo.zoneId)
// @log: Zone ID: 26
console.log('Zone balance:', zoneBalance)
...
[See the Tempo Zone Client](/tempo/client)
