...
Next, create a Viem public client for Arbitrum Sepolia and initialize a smart account. We use toSimple7702SmartAccount to treat our EOA (externally owned account) as an ERC-4337 smart account via EIP-7702 (so we don't deploy a contract upfront – the bundler will handle it if needed):
