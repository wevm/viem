...
**viem** is a TypeScript Interface for Ethereum (alternative to ethers.js & web3.js), that provides low-level stateless primitives for interacting with Ethereum. It aims to address the quadrilemma by:
...
Whereas other low-level interfaces provide _lock-in_ to opinions, **viem** wants consumers to be able to _opt-in_ to opinions (while still having sensible & safe defaults) by providing lightweight and stateless primitives & building blocks. This means that the consumer can define their own opinionated implementations (ie. [wagmi](https://wagmi.sh)) without the need for workarounds for escape hatches.
