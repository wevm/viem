...
By default, `getLogs` will include logs that [do not conform](#what-is-a-non-conforming-log) to the indexed & non-indexed arguments on the `event`, however, the `args` property will be `undefined` for non-conforming logs as we cannot deterministically decode them.
