...
import { type Chain, optimism } from '../../chains/index.js'
...
import {
  ...
  writeContractSync as writeContract,
...
async function writeContractSync<
...
test('args: value', async () => {
  ...
  const receipt_1 = await writeContractSync(clientWithAccount, {
    abi: Payable.abi,
    address: contractAddress!,
    functionName: 'pay',
  })
  const receipt_2 = await writeContractSync(clientWithAccount, {
    abi: Payable.abi,
    address: contractAddress!,
    functionName: 'pay',
    value: 1n,
  ...
  expect(
    (await getTransaction(client, { hash: receipt_1.transactionHash })).value,
  ).toBe(0n)
  expect(
    (await getTransaction(client, { hash: receipt_2.transactionHash })).value,
  ).toBe(1n)
})
...
test('w/ simulateContract', async () => {
  ...
  expect(await writeContractSync(client, request)).toBeDefined()
...
test('w/ simulateContract (args chain mismatch)', async () => {
  ...
  await expect(() => writeContractSync(client, request)).rejects
