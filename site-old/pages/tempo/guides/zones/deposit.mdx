...
### Wait for Deposit Processing

After depositing, confirm the funds have landed inside the zone with
[`zone.waitForDepositStatus`](/tempo/actions/zone.waitForDepositStatus), querying by the Tempo block number
...
`zone.waitForDepositStatus` is keyed by the Tempo block number of the deposit. Capture it from the
deposit receipt so the action can poll for processing. Use
[`zone.getDepositStatus`](/tempo/actions/zone.getDepositStatus) for a one-shot status read.
