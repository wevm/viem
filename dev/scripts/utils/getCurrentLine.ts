...
export const getFramesFromError = (error: Error): string[] => {
  ...
  try {
    ...
    try {
      ...
      // rome-ignore lint/complexity/useOptionalChain:
...
const getLocationsFromFrames = (frames: string[]): Location[] => {
  ...
  for (const frame of frames) {
    ...
    // Node can also have a error pattern of method@filepath:line:char
    const otherMatch = line.match(otherRegex)

    if (otherMatch?.groups) {
      locations.push({
        method: otherMatch.groups.method || '',
        file: otherMatch.groups.file || '',
        line: Number(otherMatch.groups.line),
        char: Number(otherMatch.groups.char),
      })
    }
  }
 ...
 */
const getLocationWithOffset = (locations: Location[], offset: Offset) => {
  // Continue
  ...
  while (i < locations.length) {
    ...
    if (
      ...
      // we are found, and we should exit immediately, so return with the frame offset applied
      if (offset.immediate) {
        // apply frame offset
        i += offset.frames || 0
      ...
      // otherwise, continue until the found condition has exited
        ...
        found = true
        ++i
        // rome-ignore lint/correctness/noUnnecessaryContinue:
        continue
      }
    }
