...
/// read or write action can surface: string reverts (`Error(string)`), panics
/// (`assert`/overflow/divide-by-zero), bare `require`, and custom errors with
/// and without arguments. `...Read` variants are `pure` (surfaced via
/// `eth_call`); `...Write` variants are state-mutating (surfaced via
/// `eth_estimateGas`). Used by the contract-error tests.
contract ErrorsExample {
    ...
    function simpleCustomWriteNoArgs() public {
        revert SimpleErrorNoArgs();
    }
