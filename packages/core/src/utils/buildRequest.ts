...
export function buildRequest<TRequest extends (args: any) => Promise<any>>(
  ...
  return (async (args: any) => {
    try {
      ...
      let err: any = err_
      if (err.code === -32700) throw new ParseRpcError(err as RpcError)
      if (err.code === -32600) throw new InvalidRequestRpcError(err as RpcError)
      if (err.code === -32601) throw new MethodNotFoundRpcError(err as RpcError)
      if (err.code === -32602) throw new InvalidParamsRpcError(err as RpcError)
      if (err.code === -32603) throw new InternalRpcError(err as RpcError)
      if (err.code === -32000) throw new InvalidInputRpcError(err as RpcError)
      if (err.code === -32001)
        throw new ResourceNotFoundRpcError(err as RpcError)
      if (err.code === -32002)
        throw new ResourceUnavailableRpcError(err as RpcError)
      if (err.code === -32003)
        throw new TransactionRejectedRpcError(err as RpcError)
      if (err.code === -32004)
        throw new MethodNotSupportedRpcError(err as RpcError)
      if (err.code === -32005) throw new LimitExceededRpcError(err as RpcError)
      if (err.code === -32006)
        throw new JsonRpcVersionUnsupportedError(err as RpcError)
