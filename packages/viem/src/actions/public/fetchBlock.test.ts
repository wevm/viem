...
test('blockNumber: fetches block by block number', async () => {
  const block = await fetchBlock(networkProvider, {
    blockNumber: initialBlockNumber - 1,
  })
  expect(block).toMatchInlineSnapshot(`
    {
      "baseFeePerGas": "0x2831941e9",
      "difficulty": "0x291a27c456bfb8",
      ...
      "timestamp": "0x62ce4223",
      "totalDifficulty": "0xb68f96ba934b89bcdee",
      "transactions": [
...
test('blockNumber: fetches block by block time (latest)', async () => {
  const block = await fetchBlock(networkProvider, {
    blockTime: 'latest',
  })
  expect(block).toBeDefined()
...
test('blockNumber: fetches block by block time (pending)', async () => {
  const block = await fetchBlock(networkProvider, {
    blockTime: 'pending',
  })
  expect(block).toBeDefined()
...
test('blockNumber: fetches block by block time (earliest)', async () => {
  const block = await fetchBlock(networkProvider, {
    blockTime: 'earliest',
  })
  expect(block).toBeDefined()
...
test('hash: fetches block by block hash', async () => {
...
test('includeTransactions: fetches block with transactions included', async () => {
  const block = await fetchBlock(networkProvider, {
    blockNumber: initialBlockNumber,
