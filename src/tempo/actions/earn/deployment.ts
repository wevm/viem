...
export type EarnFeeConfiguration = {
  ...
  excess?:
    ...
    | undefined
  /** Fixed fee recipients and rates, limited to four entries. */
  fixedFees?:
    | readonly {
        /** Fee recipient. */
        account: Address
...
>(
  ...
  try {
    ...
    if (!(await hasCode(client, predicted.earnShare))) {
      ...
      if (
        parameters.resume?.vault &&
        !isAddressEqual(parameters.resume.vault, event.args.earnVault)
      )
        throw new Error(
          'The resumed EarnVault does not match the factory deployment.',
        )
...
export namespace deployErc4626StackSync {
  ...
  > = Omit<
    WriteParameters<chain, account>,
    'gas' | 'nonce' | 'throwOnReceiptRevert'
  > &
    WriteSyncParameters<chain, account> & {
  ...
  export type ErrorType = DeployErc4626StackError | BaseErrorType
}
...
function toDeployParameters(args: createStack.Args) {
  ...
  if (transferPolicyId < 0n || transferPolicyId > 0xffffffffffffffffn)
...
async function findStackDeployment(
  ...
  const logs = await getLogs(client, {
    ...
    args: { earnShare: parameters.earnShare, earnVault: parameters.vault },
    event,
