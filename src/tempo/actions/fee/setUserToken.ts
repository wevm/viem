...
>(
  ...
  return setUserToken.inner(write, client, options)
}

export namespace setUserToken {
  export type Args = {
    /** Token to set as the default fee token: a TIP-20 token id or a contract `address`. */
  ...
  >(
    ...
    return (await action(client, {
      ...options,
      ...setUserToken.call(client, options as never),
    } as never)) as never
  }
