...
test('extend User Operation namespace', () => {
  const client = BundlerClient.create({ transport: http() }).extend(() => ({
    userOperation: {
      custom() {
        return 1 as const
      },
...
test('request: EntryPoint 0.8 schema', async () => {
  const client = BundlerClient.create({ account, transport: http() })
...
test('request: custom Ox schema preserves the Bundler schema', async () => {
  ...
  const client = BundlerClient.create({ account, schema, transport: http() })
...
test('paymaster configuration', () => {
  ...
  BundlerClient.create({ paymaster, transport: http() })
  ...
  BundlerClient.create({ paymaster: true, transport: http() })
})
