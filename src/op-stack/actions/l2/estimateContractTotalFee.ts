...
>(
  ...
  options: estimateContractTotalFee.Options<abi, functionName, args>,
): Promise<estimateContractTotalFee.ReturnType> {
  const { abi, address, args, functionName, ...request } =
    options as estimateContractTotalFee.Options
...
export declare namespace estimateContractTotalFee {
  /** Options for {@link estimateContractTotalFee}. */
  type Options<
  ...
  > = ContractFunctionParameters<
    ...
    args
  > &
    Omit<estimateTotalFee.Options, 'data' | 'to' | 'value'> &
    GetMutabilityAwareValue<
      abi,
      'nonpayable' | 'payable',
      functionName,
      NonNullable<estimateTotalFee.Options['value']>,
      args
    >

  /** Return type of {@link estimateContractTotalFee}. */
  type ReturnType = bigint

  /** Errors thrown by {@link estimateContractTotalFee}. */
  type ErrorType =
    | ContractError.fromError.ErrorType
    | AbiFunction.encodeData.ErrorType
    | AbiFunction.fromAbi.ErrorType
    | estimateTotalFee.ErrorType
