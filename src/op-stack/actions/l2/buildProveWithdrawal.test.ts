...
import { Actions, chainConfig, Withdrawal } from 'viem/op-stack'
...
function createEmptyProofServer() {
  return Http.createServer(async (req, res) => {
    ...
    req.on('end', async () => {
      ...
      const response = await fetch(anvil.local.rpcUrl.http, {
        body,
...
test('builds a legacy withdrawal proof', async () => {
  const snapshot = await CoreActions.state.snapshot(client)
  try {
    const slot = Withdrawal.getWithdrawalHashStorageSlot({
      withdrawalHash: withdrawal.withdrawalHash,
    })
    await CoreActions.address.setStorageAt(client, {
      address: chainConfig.contracts.l2ToL1MessagePasser.address,
      index: slot,
      value:
    ...
    await CoreActions.state.revert(client, { id: snapshot })
  }
})
