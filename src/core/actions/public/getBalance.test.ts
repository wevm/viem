...
test('default', async () => {
  expect(await getBalance(client, { address: weth })).toBe(wethBalance)
})
...
test('args: blockTag', async () => {
  expect(await getBalance(client, { address: weth, blockTag: 'latest' })).toBe(
    wethBalance,
  )
})
...
test('args: blockHash (EIP-1898)', async () => {
  ...
  expect(
    await getBalance(client, { address: weth, blockHash: block!.hash! }),
  ).toBe(wethBalance)
})
