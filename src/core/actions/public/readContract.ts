 ...
 *   transport: http(),
 * })
 * const balance = await Actions.readContract(client, {
...
>(
  ...
  const { abi, address, args, functionName, ...rest } =
    options as readContract.Options
  ...
  try {
    ...
    return AbiFunction.decodeResult(
      abiItem,
      response.data ?? '0x',
    ) as readContract.ReturnType<abi, functionName, args>
  } catch (error) {
...
export declare namespace readContract {
  type Options<
