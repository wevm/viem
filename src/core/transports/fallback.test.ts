...
import { fallback, rankTransports, shouldThrow } from './fallback.js'
...
describe('fallback', () => {
  ...
  test('throws when the last transport fails', async () => {
    const transport = fallback([dead(), dead()], { retryCount: 0 }).setup({})
    await expect(transport.request({ method: 'eth_chainId' })).rejects.toThrow()
  })
  ...
  test('exposes the inner transport instances', () => {
    const transport = fallback([http(url), http(url)]).setup({})
  ...
  test('rank: enabling ranking still serves requests', async () => {
    ...
    const transport = fallback([http(server.url)], {
      rank: { interval: 100 },
  ...
  test('rank: true uses defaults', async () => {
    ...
    const transport = fallback([http(server.url)], { rank: true }).setup({})
...
describe('rankTransports', () => {
  test('ranks healthy/fast transports ahead of unhealthy ones', async () => {
    const healthy = await createHttpServer((_req, res) => {
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({ result: true }))
    })
    const unhealthy = await createHttpServer((_req, res) => {
