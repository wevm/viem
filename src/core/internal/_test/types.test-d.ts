...
test('UnionToTuple', () => {
  // NOTE: tuple order is an unstable TS-checker artifact (instantiation
  // encounter order) — assert length + membership instead of order.
  expectTypeOf<UnionToTuple<'foo' | 'bar'>['length']>().toEqualTypeOf<2>()
  expectTypeOf<UnionToTuple<'foo' | 'bar'>[number]>().toEqualTypeOf<
    'foo' | 'bar'
  >()
})
