...
describe('approve', () => {
  ...
  test('call: parses amount with explicit decimals', () => {
  ...
  test('call: infers decimals from chain tokens', () => {
    const call = approve.call(client, { token: usdc, spender, amount: '1' })
    expect({
      data: call.data,
      functionName: call.functionName,
      to: call.to,
  ...
  test('decimals: defaults to 0 for an undeclared token', async () => {
    ...
    // passed, so `decimals` defaults to 0: `amount: '1'` approves `1n`.
