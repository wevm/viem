...
const daiHolder = address.daiHolder

beforeAll(async () => {
  await prepareAccount(holder)
  await prepareAccount(spender)
  await prepareAccount(daiHolder)
})
...
describe('transfer', () => {
  ...
  test('call: infers decimals from chain tokens', () => {
    const call = transfer.call(client, { token: usdc, to, amount: '1' })
    expect({
      data: call.data,
  ...
  test('call: defaults decimals to 0 for an undeclared token', () => {
    // DAI is not declared on the chain's `tokens` config, so `decimals`
    // defaults to 0: `amount: '1'` parses to `1n` base units.
  ...
  test('decimals: defaults to 0 for an undeclared token', async () => {
    ...
    const receipt = await getTransactionReceipt(client, { hash })
    const { args } = transfer.extractEvent(receipt.logs)
    expect(args.value).toMatchInlineSnapshot(`1n`)

    const after = await getBalance(client, { token: dai, account: recipient })
    expect(after.amount - before.amount).toMatchInlineSnapshot(`1n`)
  })
