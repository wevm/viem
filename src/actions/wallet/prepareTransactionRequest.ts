...
> {
  ...
  const attemptFill = (() => {
    ...
    // Relay sponsorship requires fill even when transaction fields are complete.
    // Local fee payers sign during serialization; empty parameter lists only prepare internal estimates.
    if (
      parameters.length > 0 &&
      'feePayer' in request &&
      (request as any).feePayer === true &&
