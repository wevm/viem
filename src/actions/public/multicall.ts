...
import { deploylessMulticall3Bytecode } from '../../constants/bytecodes.js'
...
> = Pick<CallParameters, 'blockNumber' | 'blockTag'> & {
  ...
  /** Whether or not to fall back to a [deployless multicall3](https://destiner.io/blog/post/deployless-multicall/) implementation. */
  unstable_deployless?: boolean
}
...
>(
  ...
  const deployless =
    unstable_deployless ??
    ((typeof client.batch?.multicall === 'object' &&
      client.batch.multicall.unstable_deployless) ||
      false)
  ...
  const aggregate3Results = await aggregate3({
    calls: chunkedCalls,
    client,
    multicallAddress,
    blockNumber,
    blockTag,
    unstable_deployless: deployless,
  })
...
>({
  ...
  if (!multicallAddress_) {
    ...
    chainMulticallAddress = getChainContractAddress({
      ...
      chain: client.chain,
      contract: 'multicall3',
      allowMissing: unstable_deployless,
    })
  }

  const multicallAddress = multicallAddress_ || chainMulticallAddress

  // Fallback to deployless, if enabled.
  if (!multicallAddress && unstable_deployless) {
