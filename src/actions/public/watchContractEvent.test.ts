...
import { accounts, address, publicClient, testClient, usdcContractConfig, walletClient } from '../../_test';
...
test('args: args unnamed', async () => {
  ...
  expect(logs[0][0].args).toEqual([getAddress(address.vitalik), getAddress(accounts[0].address), 1n]);
  expect(logs[0][0].eventName).toEqual('Transfer');
  expect(logs[0][1].args).toEqual([getAddress(address.vitalik), getAddress(accounts[1].address), 1n]);
  expect(logs[0][1].eventName).toEqual('Transfer');
});
...
test(
  ...
  async () => {
    ...
    vi.spyOn(createContractEventFilter, 'createContractEventFilter').mockRejectedValueOnce(new Error('foo'));
...
describe('errors', () => {
  test('handles error thrown from creating filter', async () => {
    vi.spyOn(getBlockNumber, 'getBlockNumber').mockRejectedValueOnce(new Error('foo'));
    vi.spyOn(createContractEventFilter, 'createContractEventFilter').mockRejectedValueOnce(new Error('foo'));
  ...
  test(
    ...
    async () => {
      vi.spyOn(getFilterChanges, 'getFilterChanges').mockRejectedValueOnce(new Error('bar'));
