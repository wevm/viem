...
import * as Address_ox from 'ox/Address'
...
export async function verifyHash<chain extends Chain | undefined>(
  ...
  // We will attempt to coerce the signature into a signature envelope to verify.
  const envelope = (() => {
    try {
  ...
  // If coercion was successful, we can directly verify the envelope (no need for a network request).
  if (envelope && envelope.type !== 'secp256k1')
    return await verifySignatureEnvelope(envelope, parameters)
...
export async function verifySignatureEnvelope(
  ...
  const address = (() => {
    if (envelope.publicKey) return Address_ox.fromPublicKey(envelope.publicKey)
    return undefined
  })()

  if (!address) return false
  if (!isAddressEqual(address, parameters.address)) return false
  ...
  throw new BaseError(
    `signature type \`${(envelope as { type: string }).type}\` is not supported`,
  )
}
