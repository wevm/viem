...
test('error: scientific notation', () => {
  expect(() => parseUnits('1.234e5', 3)).toThrowErrorMatchingInlineSnapshot(`
    [ScientificNotationUnsupportedError: Scientific notation is not supported for unit parsing (value: \`1.234e5\`). Please provide a decimal number.

    Version: viem@x.y.z]
  `)
  expect(() => parseUnits('1.234e-5', 3)).toThrowErrorMatchingInlineSnapshot(`
    [ScientificNotationUnsupportedError: Scientific notation is not supported for unit parsing (value: \`1.234e-5\`). Please provide a decimal number.
