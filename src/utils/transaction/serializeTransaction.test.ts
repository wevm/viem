...
test("default values", () => {
  expect(serializeTransaction({ chainId: 1 }, { type: "eip1559" })).toEqual(
    serializeTransactionEthers({ chainId: 1, type: 2 })
  );

  expect(serializeTransaction({ chainId: 1 }, { type: "eip2930" })).toEqual(
    serializeTransactionEthers({ chainId: 1, type: 1 })
  );

  expect(serializeTransaction({ chainId: 1 }, { type: "legacy" })).toEqual(
    serializeTransactionEthers({ chainId: 1 })
  );
});
