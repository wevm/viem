...
test('cleans up cache entries when all listeners unsubscribe', async () => {
  ...
  // Caches should have entries after observing
  expect(listenersCache.has(id)).toBe(true)
  expect(listenersCache.get(id)!.length).toBe(2)

  // Unwatch first listener
  unwatch1()
  expect(listenersCache.has(id)).toBe(true)
  expect(listenersCache.get(id)!.length).toBe(1)

  // Unwatch second listener - should clean up caches
  unwatch2()
  expect(listenersCache.has(id)).toBe(false)
  expect(cleanupCache.has(id)).toBe(false)
})

test('cleans up cache entries for multiple observer ids', async () => {
  const id1 = 'test-cleanup-1'
  const id2 = 'test-cleanup-2'
  const callback1 = vi.fn()
  const callback2 = vi.fn()
  ...
  const unwatch1 = observe(id1, { emit: callback1 }, emitter)
  const unwatch2 = observe(id2, { emit: callback2 }, emitter)

  await wait(100)

  expect(listenersCache.has(id1)).toBe(true)
  expect(listenersCache.has(id2)).toBe(true)

  unwatch1()
  expect(listenersCache.has(id1)).toBe(false)
  expect(listenersCache.has(id2)).toBe(true)

  unwatch2()
  expect(listenersCache.has(id1)).toBe(false)
  expect(listenersCache.has(id2)).toBe(false)
})
