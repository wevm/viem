...
import { LEGACY_ETH_ADDRESS } from '../constants/address.js'
import { getBalanceOfTokenL1, type BalanceOfTokenL1Parameters } from './getBalanceOfTokenL1.js'
...
>(
  ...
  const token = token_ ?? LEGACY_ETH_ADDRESS;

  if (isETH(token)) {
    return await getBalance(client, { address: account!.address, blockTag: blockTag });
  }
