...
// Freeze time so validBefore assertions are deterministic.
...
describe('sendSponsoredCalls (end-to-end)', () => {
    ...
    return createClient({
      ...
      transport: custom({
        async request({ method }: { method: string }) {
          ...
          // Actor not yet bound → `getActorConfig` returns an all-zero 3-word
          // ActorConfig struct, so `resolveSigningScope` falls back to the
          // declared handle scope (offline).
  ...
  test('full sponsorship: sender-signs, payer relays; expiry = now + maxExpiry', async () => {
    ...
    // validBefore = now + maxExpiry (unix ms); frozen clock ⇒ deterministic.
    expect(parsed.validBefore).toBe(
      BigInt(FROZEN_NOW_MS) + BigInt(MAX_EXPIRY_REL) * 1000n,
    )
  })
