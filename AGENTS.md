...
- **Action decorators are named exports** -- expose decorator factories as named exports such as `publicActions`, `walletActions`, `ensActions`, `contractActions`, and `testActions`. Use `Client.create(...).extend(actions.publicActions())`, not callable namespace exports.
- **Decorator actions use named action-map types** -- when a decorator attaches methods to a client namespace, declare and export a named type for that decorated method map, e.g. `PublicActions` or `TestActions`, with TSDoc on each method. Return objects typed by that map so editor docs on `client.public.*`, `client.wallet.*`, and related namespaces stay specific.
