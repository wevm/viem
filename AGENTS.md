...
- **Alias `client.test` in test files** -- Vitest's collector treats call expressions containing `.test` as potential test declarations. In `*.test.ts` and `*.test-d.ts`, assign `const clientTest = client.test` before calling decorated test action methods.
