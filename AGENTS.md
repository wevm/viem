...
- **Action generics follow client slots** -- when an action needs client generics, expose the specific client slots instead of a whole `client extends Client.Client` generic. Order them as `chain`, `account`, `transport`, `schema`, then action-specific generics. Add only the slots the action needs.
