...
- **Avoid `as` at all costs** -- never reach for `as T` to silence the type checker. Prefer fixing the shape (widen a `readonly` array via spread `[...xs]`, declare a more accurate option type, use a generic, extract an IIFE that returns a precisely typed value). A type assertion is only acceptable when an external type genuinely cannot express the runtime contract (e.g. an Ox RPC schema missing a spec-defined branch). When you must keep one, contain it to the smallest possible expression and add a one-line comment explaining why it is unavoidable.
- **Prefer IIFE over long ternaries** -- when a local value needs more than a single ternary branch (nested ternaries, mixed branches, or a multi-line conditional value), prefer an IIFE expression with `if` / `return` statements over chained `?:`. The IIFE rule from "fallible local derivations" extends to any local whose derivation is too wide for a clear ternary.
...
- **Never construct clients in tests** -- do not call `Client.create(...)` with `http(anvil.rpcUrl.http)` directly in tests. Use the `anvil.getClient(anvilFoo, options?)` helper from `test/anvil.ts` so the chain, transport, and other defaults stay consistent across the suite. Import the helper through a namespace import (`import * as anvil from '../../../test/anvil.js'`) alongside the named chain import (`import { anvilMainnet } from '../../../test/anvil.js'`) so it reads as `anvil.getClient(anvilMainnet)`. Adding extra client options is fine via the second argument (`anvil.getClient(anvilMainnet, { blockTag: 'earliest' })`). The only exception is a deliberately non-Anvil client used to capture RPC shapes via `custom(...)`; treat that as a localized testing tool, not the default.
- **Dogfood actions over raw RPC** -- do not call the low-level `request(anvil, method, params)` helper from `test/anvil.ts` in test bodies. Use the corresponding action (`actions.setBalance`, `actions.mine`, `actions.getBlock`, etc.) so tests exercise the public surface. When no action exists yet (e.g. wallet actions during the v3 buildout), fall back to `client.request({ method, params })` so the call still flows through the client transport stack, and leave a one-line comment noting the deferred action.
...
- **Canonical root imports** -- the v3 root public shape is `import { Account, BaseError, Chain, Client, Transport, http, publicActions, testActions } from 'viem'`. Root exports PascalCase viem-owned modules, the special `BaseError`, flat transport factories, and root action decorators.
...
- **Client generics follow slots** -- order `Client.Client` and related client generics as `chain`, `account`, `transport`, `schema`, then `extended`.
...
- **Actions are named exports** -- expose standalone actions as flat named exports from `viem/actions`, e.g. `actions.getBalance`, `actions.sendTransaction`, `actions.getAddress`, `actions.read`, and `actions.mine`. Avoid namespace objects that attach actions with `Object.assign`; they are too easy to make non-tree-shakable.
- **Decorated client action namespaces are fixed** -- decorators attach core actions under `client.public`, `client.wallet`, `client.contract`, `client.ens`, and `client.test`. Do not split blocks, filters, or transactions into extra decorated namespaces unless the API direction changes explicitly.
- **Action decorators are named exports** -- expose decorator factories as named exports such as `publicActions`, `walletActions`, `ensActions`, `contractActions`, and `testActions`. `publicActions` and `testActions` are also exported from root `viem`. Use `Client.create(...).extend(publicActions())`, `Client.create(...).extend(testActions())`, or the same decorators from `viem/actions`; do not use callable namespace exports.
- **Action decorators preserve client slots** -- decorator factories return generic decorators over the client slots they accept. Keep the order `chain`, `account`, `transport`, `schema` so extending a client does not widen or erase its configured slots.
- **Decorator actions use named action-map types** -- when a decorator attaches methods to a client namespace, declare and export a named type for that decorated method map, e.g. `PublicActions` or `TestActions`. Return objects typed by that map so editor docs on `client.public.*`, `client.wallet.*`, and related namespaces stay specific.
- **Decorator action docs are complete** -- every method on a decorator action-map type gets TSDoc with a summary, `@example`, `@param` for each parameter, and `@returns`. Method return annotations should reference the action namespace return type, e.g. `getBlockNumber.ReturnType`, not `ReturnType<typeof getBlockNumber>`.
- **Test actions support node modes** -- v3 test actions keep Anvil, Hardhat, and Ganache RPC branches. Do not reintroduce `createTestClient({ mode })`; pass mode through `testActions({ mode })` for decorated clients or action options for standalone calls.
- **Action generics follow client slots** -- when an action needs client generics, expose only the specific client slots that affect the public action type. Order them as `chain`, `account`, `transport`, `schema`, then action-specific generics. Do not add unused slots just to mirror `Client.Client`.
...
- **Remove redundant domain words** -- when the action group already names the domain, remove the repeated word from the method name: `getEnsAddress` becomes `actions.getAddress` and `client.ens.getAddress`, `readContract` becomes `actions.read` and `client.contract.read`, `simulateContract` becomes `actions.simulate` and `client.contract.simulate`, and `multicall` becomes `actions.multicall` and `client.contract.multicall`.
...
- **Clients are created through `Client.create`** -- replace `createClient`, `createPublicClient`, `createWalletClient`, and `createTestClient` with `Client.create(...).extend(publicActions())` and related decorators.
