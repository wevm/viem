...
- **Canonical root imports** -- the v3 root public shape is `import { Account, BaseError, Chain, Client, Transport, http, publicActions } from 'viem'`. Root exports PascalCase viem-owned modules, the special `BaseError`, flat transport factories, and the public action decorator.
...
- **Action decorators are named exports** -- expose decorator factories as named exports such as `publicActions`, `walletActions`, `ensActions`, `contractActions`, and `testActions`. `publicActions` is also exported from root `viem`. Use `Client.create(...).extend(publicActions())` or `Client.create(...).extend(actions.publicActions())`, not callable namespace exports.
...
- **Test actions support node modes** -- v3 test actions keep Anvil, Hardhat, and Ganache RPC branches. Do not reintroduce `createTestClient({ mode })`; pass mode through `actions.testActions({ mode })` for decorated clients or action options for standalone calls.
...
- **Clients are created through `Client.create`** -- replace `createClient`, `createPublicClient`, `createWalletClient`, and `createTestClient` with `Client.create(...).extend(actions.publicActions())` and related decorators.
