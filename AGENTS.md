...
- **Test actions are Anvil-native** -- v3 test actions target the shared Anvil test harness first. Do not reintroduce `createTestClient({ mode })` or Hardhat/Ganache mode branching unless that backend is explicitly reviewed.
